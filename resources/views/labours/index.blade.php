                        ...
                        <form class="row g-2 align-items-center" method="get" action="" id="filter-form">
                            ...
                            <div class="col-4 foat-end">
                                <div class="btn-group" role="group" aria-label="Source Filter">
                                    <a href="?status={{ request('status', 'all') }}&source=all" class="btn btn-outline-secondary btn-sm @if(request('source', 'all') == 'all') active @endif">ทั้งหมด</a>
                                    <a href="?status={{ request('status', 'all') }}&source=api" class="btn btn-outline-primary btn-sm @if(request('source') == 'api') active @endif">API Import</a>
                                    <a href="?status={{ request('status', 'all') }}&source=manual" class="btn btn-outline-success btn-sm @if(request('source') == 'manual') active @endif">Manual Entry</a>
                                </div>
                            </div>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    <img src="{{ $row->labour_image_thumbnail_path ? asset('storage/' . ltrim($row->labour_image_thumbnail_path, '/')) : asset('images/user_icon.png') }}" class="rounded-circle" style="width:30px;height:30px;object-fit:cover;">
                                    <div style="font-size:0.85em;color:#888;">{{ $row->labourStatus->value ?? '-' }}</div>
                                </td>
                                <td>{{ $row->labour_idcard_number }}</td>
                                <td>{{ $row->labour_prefix }}. {{ $row->labour_firstname }} {{ $row->labour_lastname }}</td>
                                <td class="text-center">
                                    @if(!empty($row->api_candidate_id) || !empty($row->api_imported_at) || $row->source_type === 'api')
                                        <span class="badge bg-primary"><i class="fas fa-cloud-download-alt me-1"></i>API Import</span>
                                    @else
                                        <span class="badge bg-success"><i class="fas fa-user-plus me-1"></i>Manual Entry</span>
                                    @endif
                                </td>
