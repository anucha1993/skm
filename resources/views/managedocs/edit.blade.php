...
@section('content')
<div class="col-sm-12">
    <form action="{{ route('managedocs.update', $managedoc->managedoc_id) }}" method="POST">
        @csrf
    @method('PUT')
  
        <div class="card">
            <div class="card-header">
                <h5>แก้ไข การจัดการเอกสาร</h5>
            </div>
            ...
            <div class="card-body">
                    ...
                    <div class="col-md-12">
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <input type="text" class="form-control" id="managedoc_name" name="managedoc_name" value="{{$managedoc->managedoc_name}}"
                            ...
                            <div class="col-sm-9">
                                <select name="managedoc_country" class="form-control" id="managedoc_country">
                                    ...
                                    @forelse ($country as $item)
                                        <option @if($managedoc->managedoc_country === $item->country_job_id) selected @endif value="{{ $item->country_job_id }}">{{ $item->country_job_name }}</option>
                                    @empty
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <select name="managedoc_status" class="form-control" id="managedoc_status">
                                    <option @if($managedoc->managedoc_status === 'Y') selected @endif value="Y">เปิดใช้งาน</option>
                                    <option @if($managedoc->managedoc_status === 'N') selected @endif value="N">ปิดใช้งาน</option>
                                </select>
                            </div>
                        </div>
                    </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <table class="table table-success" id="documentTable">
                        ...
                        <tbody>
                            @forelse ($managedoc->managefiles as $item)
                                
                          
                            <tr>
                                <td style="width: 100px">
                                    <input type="number" name="managefile_no[]" class="form-control form-control-sm" value="{{$item->managefile_no}}" readonly>
                                </td>
                                <td style="width: 300px">
                                    <input type="text" name="managefile_code[]" class="form-control form-control-sm" value="{{$item->managefile_code}}" placeholder="CV" required>
                                <td>
                                    <input type="text" name="managefile_name[]" class="form-control form-control-sm" value="{{$item->managefile_name}}" placeholder="เอกสาร-CV" required>
                                </td>
                                <td>
                                    <select name="managefile_step[]" class="form-control form-control-sm" required>
                                        <option @if($item->managefile_step === 'A') selected @endif value="A">Step A </option>
                                        <option @if($item->managefile_step === 'B') selected @endif value="B">Step B </option>
                                    </select>
                               
                                </td>
                                <td><input type="checkbox" name="managefile_status[]" value="{{$item->managefile_status}}" @if($item->managefile_status === 'Y') checked @endif ></td>
                                <td>
                    ...
                    <button type="submit" class="btn btn-primary mt-3 float-right">ยืนยันการแก้ไข</button>
                </div>
            </div>
        </div>
